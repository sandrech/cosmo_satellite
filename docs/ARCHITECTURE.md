...
## Интерактивный расчёт временного контекста

Для длинной временной сетки UI не обязан ждать последовательный расчёт от
`t=0` до конца периода. После запуска используется двунаправленный WebSocket
`/api/model/run/context`.

Frontend отправляет текущий focus timeline (`t_s`), направление движения и
радиус контекста. Backend хранит рассчитанные `DynamicFrame` по индексам сетки
и выбирает следующую работу через `ContextFrameScheduler`:

1. точный кадр под focus;
2. ближайшие кадры внутри focus-window;
3. при равном расстоянии — небольшой приоритет в сторону движения timeline;
4. остальная сетка постепенно досчитывается в фоне.

Если focus меняется во время дорогого кадра, уже начатый кадр завершается и
сохраняется, но остаток старого батча вытесняется новым приоритетом. Поэтому
прыжок по timeline задерживается максимум на текущую атомарную работу, а не на
весь ранее запланированный батч.

Протокол сервера: `start -> frames* -> phase(aggregating) -> complete`.
`frames` содержит явные `indices`, поэтому частичный trace является разреженным
и не считается непрерывным префиксом от нуля. После получения всех кадров
backend сортирует их в каноническом порядке сетки и вызывает тот же
`DynamicModel.analyze_frames`, что и обычный полный расчёт. Таким образом
приоритетный порядок вычисления не меняет математический результат.
