...
#table(
  ...
  table.header([Стратегия], [Стоимость ребра], [Что минимизирует]),
  table.hline(stroke: 0.6pt + ink),
  [`minimum_hops` (основная)], [$w(e) = 1$], [число переходов, включая обе наземные линии],
  [`minimum_distance`], [$w(e) = d_e$], [суммарную длину линий в километрах],
  table.hline(stroke: 0.4pt + hair),
)

#rule[Выбор маршрута][
  $ P_tau = op("arg min", limits: #true)_(P : c ~> tau "в" G_c) J(P), quad J(P) = sum_(e in P) w(e), quad tau in T_c $
  $ P^* = op("arg min", limits: #true)_(tau in T_c) ( J(P_tau), space "hop"(P_tau), space tau, space P_tau ) quad "лексикографически" $
]

Для каждого достижимого шлюза кратчайший путь ищется алгоритмом Дейкстры, веса неотрицательны, стоимость $O((abs(V) + abs(E)) log abs(V))$. Среди шлюзов выбирается путь с наименьшей стоимостью, при равенстве с меньшим числом переходов, затем по идентификаторам: результат детерминирован.

Маршрут хранится как последовательность узлов и сегментов (тип линии, длина, угол возвышения наземного сегмента) с показателями $"hop"(P) = abs(E(P))$, $"len"(P) = sum_(e in P) d_e$ и $J(P)$. Путь основной стратегии выгружается в результат `cosmo-A-result-1.0`: одна запись $(t_i, c, P^*)$ на каждую пару отсчёта и клиента, пустой список при отсутствии пути.
...
#table(
  ...
  [маршрут каждой стратегии], [потерян, изменён список узлов, рост стоимости #box[$J(P^(-s)) - J(P)$]],
  table.hline(stroke: 0.4pt + hair),
)
...
Здесь $bot$ означает отсутствие маршрута. Перерыв разрывает непрерывность: восстановление после него начинает новый эпизод, но переключением не считается. Для каждой стратегии сохраняются маршрут на каждом отсчёте, эпизоды, переключения и статистика $"hop"$, $"len"$ и $J$ по отсчётам с маршрутом.
...
#table(
  ...
  [3.3], [#code("static_model")], [`RouteStrategy`, `HopCountCost`, `DistanceCost`, `StaticModel.select_route`],
  [3.4, 3.5], [#code("static_model")], [`satellite_connectivity`, `SatelliteFailureImpact`, `LexicographicCriticalityRanking`],
  [3.6, 3.7, 3.8], [#code("dynamic_model")], [`TimeGrid`, `DynamicModel.analyze`, `NodePathRouteIdentity`],
  [3.9], [#code("dynamic_model")], [`SatelliteTemporalCriticality`, `LexicographicDynamicCriticalityRanking`],
  [3.10], [#code("variant_comparison")], [`VariantComparator`, `cosmo_a_comparison_adapter`],
