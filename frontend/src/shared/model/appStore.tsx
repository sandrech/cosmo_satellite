...
export function AppStateProvider({ children }: PropsWithChildren) {
  ...
  const runScenario = useCallback(async (
    ...
    setError(null);
    try {
      const run = await api.runModel(nextScenario, nextRoutingStrategy);
      setModelRun(run);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (preferred) {
          const nextScenario = await api.getModel(preferred.id);
          if (!active) return;
          setScenarioState(nextScenario);
          setClientId(firstClientId(nextScenario));
          ...
          const run = await api.runModel(DEFAULT_SCENARIO, "minimum_hops");
          if (!active) return;
          setModelRun(run);
          setRoutingStrategies(run.routingStrategies.length ? run.routingStrategies : DEFAULT_ROUTING_STRATEGIES);
        }
