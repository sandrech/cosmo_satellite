...
- `static_model.StaticAnalysis` ↔ `static-analysis-2.0`
- `dynamic_model.DynamicAnalysis` → `dynamic-analysis-2.0`
- `variant_comparison.VariantComparisonReport` → `variant-comparison-2.0`
...
contracts, so duplicating every raw spatial snapshot inside `dynamic-analysis-2.0`
